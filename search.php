    ...
    if ($result) {
        if(mysqli_num_rows($result) > 0){
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                echo '
                        <tr onclick="location.href=\'http://localhost\Ecommerse\Ecommerce_Web/item.php?sno='.$row['sno'].'\'"style="cursor: pointer;">
                                <td>'.$sno++.'</td>
                                <td>'.$row['item_name'].'</td>
                                <td>'.$row['item_size'].'</td>
                                <td>'.$row['item_rate'].'</td>
                                <td><center><img src="http://localhost\Ecommerse\Ecommerce_Web/upload/uploads/'.$row['item_img_1'].'" width=100px height=100px></center></td>
                        </tr>
                ';
            }
        }
