...
<body>
  ...
  document.getElementById('locate-btn').addEventListener('click', function() {
    map.setView(["//shop_location"], 16); // Pan to the shop location with zoom level 16
  });
    var map = L.map('map').setView(["shop_location"], 13); // Default center at shop location
    ...
    var redIcon = L.icon({
      iconUrl: 'icon url',//Enter the icon url in the '' 
      shadowUrl: 'URL',// Enter the shado img url in the ''  
      iconSize: [25, 41],
      iconAnchor: [12, 41],
      popupAnchor: [1, -34],
      shadowSize: [41, 41]
    ...
    var shopMarker = L.marker([27.159653, 77.916266], { icon: redIcon }).addTo(map)
      .bindPopup('ms Handicraft in Agra, UP')
      .openPopup();
